...
ngx.say("Nginx Worker PID: ", ngx.worker.pid() ,  " count: ", ngx.worker.count() , "<br>")
...
-- end
-- local succ, err, forcible  = upstream_share:flush_all();
local function isempty(s)
  return s == nil or s == ''
end

if isempty(i_stream) or isempty(i_name) or isempty(i_act) then
    ngx.print("Parameter error")
    for kk , vv in pairs(upstream_share:get_keys(1000)) do
...
for _, u in ipairs(us) do
    ...
    if type(upstream_name) == "string" and upstream_name == i_stream  then
        ngx.say("find upstream =>  ", upstream_name , "<br>")
        if not srvs then
            ...
            for _, srv in ipairs(srvs) do
                for k, v in pairs(srv) do
                    if type(v) == "table" then
                        ...
                        if type(v) == "string" and v == i_name  and first then
                            first = false  -- 무조건 한번만 매칭되게 하기위함
                            ngx.print( " <br> (" .. count .. ") Find out - " ..  v  .. " / " ..  upstream_name .. "\n <br>" )
                            local rnd = math.random(os.time());
                            if type(v) == "string" and i_act == "down"  then
                                local upstream_ctl, err = upstream.set_peer_down(upstream_name, false, count, true)
                                -- ngx.say ("(" .. count .. ") act : " , i_act , " / ",  err,  "<br> \n")
                                --for k , worker_id in pairs(worker_pid_dict:get_keys(100)) do
                                for k=0, ngx_worker_count - 1 do
                                    local worker_pid = ngx.shared.worker_pid_dict:get(k)
                                    local dic_key = worker_pid .. "," .. upstream_name .."," ..  v .. "," .. count .. "," .. rnd
                                    -- ngx.print ("upstream shared dict -> " .. k .. "  / "..  dic_key .. "<br>")
                                    local succ, err, forcible  = upstream_share:set( dic_key ,"down" ,3 )
                                    if err then
                                        ngx.log(ngx.ERR, "PID: " .. worker_pid .. " / upstream_share:set() error: ", err)
                                        return
                                    end
                                end
                            -- elseif type(v) == "string" and string.match( i_act , "up" ) then
                            elseif type(v) == "string" and i_act == "up"  then
