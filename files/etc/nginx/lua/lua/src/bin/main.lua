  ...
  if frontend == nil then
    ...
    if session['servers'] == nil then
      ...
      if session['server'] ~= nil then
        ...
        library.log("JJJ_ SERVER: " .. session['server'])
        
        local res = ngx.location.capture(session['server'])
        library.log("JJJ_ RES: " .. res.status)

                
        
        
        --library.log(ngx.ERR, 'JJJJJ : ', ngx.var.upstream)
        -- ngx.log(ngx.ERR, 'JJJJJ : ', ngx.var.upstream)
